...
| 017 | 2026-04-15 | 반기 뱃지: `get_player_titles` RPC (실시간 계산, 최소 3경기, 공동 1위) | 확정 |
| 018 | 2026-04-15 | 프로필카드 사각 + 타 화면 원형 + 업로드 단계에 인스타식 크롭 | 확정 |
| 019 | 2026-04-15 | 홈 참가 버튼: 짧게탭 = 자동 참가(프로필 선호포지션), 길게누름 = 시트 | 확정 |
...
- 이름/등번호/포지션/발/키 등 나머지 프로필 필드 저장은 이번 변경 scope 밖
  (피커/업로드만 실제 연결, 텍스트 필드는 여전히 pop) — **결정 018 에서 해결**

## 017. 반기 뱃지 — `get_player_titles` RPC (실시간 계산)

**결정**: 선수의 반기 뱃지(득점왕/어시왕/출석왕/MOM왕)는 사전 적재 테이블이
아니라 **DB RPC 가 요청 시점에 CTE 로 계산**. 팀 스코프, 반기('H1'=1~6월,
'H2'=7~12월), 최소 3경기, **공동 1위 전원 수여**.

**이유**:
- 팀 50~70명 × 경기 주 1~2회 규모에선 조회 빈도 대비 적재 비용이 비쌈
- 반기 진행 중엔 매 경기 결과마다 뱃지가 바뀔 수 있어 트리거/배치 재계산
  부담이 더 큼. 실시간 계산이 단순하고 항상 정확
- 읽기 속도는 CTE 1회 ~수 ms. 프로필 조회 수준 빈도엔 충분

**구현**:
- `supabase/migrations/20260415020000_player_titles.sql`:
  `get_player_titles(p_player_id, p_team_id, p_year, p_half)` →
  `player_match_stats JOIN matches` → 그룹바이 player → HAVING ≥ 3경기 →
  `max_vals` CTE 1행 → 4 CASE 분기로 내 값 vs max 비교 → 라벨 배열 반환
- 알파 사례: 아직 아무도 MOM 없을 때 `max_mom = 0` 이면 수여 안 함
- `SeasonHalf.first/second` → `'H1'/'H2'` 매핑은 `SupabaseStatsRepo.getPlayerTitles`
  에서 담당 (뷰 스키마의 `'상반기'/'하반기'` 와 구분)

**트레이드오프**:
- 현재 반기 종료 후 "역대 뱃지" 보관은 없음. 반기 말 스냅샷 테이블
  (`player_title_history`) 은 별도 결정에서 도입 예정
- RPC 가 UI 라벨('득점왕' 등) 을 직접 반환 — i18n 시 `top_scorer` 같은
  안정 코드로 분리 필요

**뱃지 카테고리**:
| 카테고리 | 측정 | 최소 자격 |
|---|---|---|
| 득점왕 | `SUM(goals)` 팀 내 1등 (> 0) | 3경기 |
| 어시왕 | `SUM(assists)` 팀 내 1등 (> 0) | 3경기 |
| 출석왕 | `COUNT(DISTINCT match_id)` 팀 내 1등 | 3경기 |
| MOM왕 | `COUNT(*) FILTER (is_mom)` 팀 내 1등 (> 0) | 3경기 |

## 018. 프로필카드 사각 + 타 화면 원형 + 업로드 단계 인스타식 크롭

**결정**: 업로드된 아바타는 **단일 정사각 이미지**로 저장. 렌더 시:
- **프로필카드**: 220×220 사각형(`ClipRRect(AppRadius.smoothMd)`) — 크롭한
  전체 영역 노출
- **나머지(멤버 목록/채팅/DM/홈 프로필 아이콘 등)**: 기존 `ClipOval` 유지

업로드 단계에서 **인스타그램 스타일 크롭 스크린**을 강제:
- 320×320 고정 크롭 박스, InteractiveViewer 로 드래그/핀치
- **사각 테두리 + 인스크립트 원** 가이드 동시 표시 → 사각(프로필카드) 와
  원(아바타) 노출 범위 동시 예측 가능
- 3분할 가이드 라인
- `RepaintBoundary.toImage(pixelRatio: 3)` → PNG 업로드

**이유**:
- 프로필카드는 유저의 "본인 대표 이미지" 가 가장 크게 노출되는 화면 —
  원형 clip 으로 잘리면 어색한 경우 많음. 사각 전체 노출이 나음
- 동시에 다른 화면의 원형 아바타는 UI 관습상 유지 — 같은 이미지를 두
  방식으로 활용
- 업로드 시점에 사용자가 두 결과를 모두 예측하려면 가이드가 필요 — 인스타
  업로드 UI 의 원+사각 프레임이 적절한 레퍼런스

**구현**:
- `lib/features/profile/presentation/widgets/crop_image_screen.dart` 신규
- `pickTeamLogoImage(context, maxWidth: 1500, imageQuality: 92)` 로 픽 후 push
- 결과는 항상 PNG (ext = 'png' 하드코딩) — `ui.ImageByteFormat.png` 고정
- `lib/core/utils/capture.dart` 의 `captureWidgetAsPng` 공용화
  (share + crop 재사용, ui.Image.dispose 누수 방지)

**보너스** (같은 커밋에서 정리):
- `SupabaseProfileRepo.update` 에 `.select('id')` + `result.isEmpty` 검증 —
  RLS 가 0 rows 로 조용히 무시하는 케이스를 StateError 로 surface
- `_EditProfileScreenState` 가 전 필드(name/number/positions/foot/height) 를
  실제 저장하도록 구현 완료 (결정 016 메모의 미완 항목 해결)
- 편집 state 가 `Set<Position>` / `PreferredFoot` 직접 보유 (기존 String
  라운드트립 제거)

**트레이드오프**:
- 크롭 출력 ~960×960 PNG — 평균 300~800KB 로 다소 큼. pixelRatio 2 로
  낮추면 반감 가능, 관찰 후 조정 (결정 016 의 아바타 orphan 이슈와 함께)
- 구 아바타 파일 자동 정리 여전히 없음 (별도 크론 필요)

## 019. 홈 참가 버튼: 짧게탭 = 자동 참가, 길게누름 = 시트

**결정**: 홈 다음경기 카드의 "참가하기" 버튼 제스처 이원화.
- **짧게 탭** → 프로필의 `preferredPositions` + 전 쿼터(1~4) 로 바로
  `MatchService.joinMatch` 호출. 시트 안 뜸
- **길게 누름** → 기존 `showJoinMatchSheet` (포지션/쿼터 커스텀)
- **이미 참가** 상태에서 짧게 탭 → `leaveMatch` 로 참가 취소
- 프로필에 선호 포지션 비어있으면 SnackBar 로 프로필 설정 유도
- `_busy` 플래그로 중복 호출 방지

**이유**:
- 대다수 참가는 "항상 내 포지션으로, 전 쿼터 참가" — 시트까지 띄우는 것은
  마찰. 탭 한 번으로 참가 완료가 압도적으로 빠름
- 커스텀이 필요한 경우(포지션 바꿔서 참가, 특정 쿼터만)에만 long-press 로
  기존 UI 제공 — 발견성은 낮지만 사용 빈도도 낮음
- 기존 로컬 `_isJoined` 토글은 DB 저장 안 하는 dummy 였음. 이번 변경으로
  실제 `match_participations` 저장 (장기 과제였던 "실제 참가 연동" 해결)

**부수 fix**:
- `join_match_sheet.dart` 의 내부 `_quarters` 가 0-indexed 였고 호출자가
  `q + 1` 변환하는 leaky 구조 → 시트 내부도 1-indexed 로 통일
  (`JoinMatchResult.availableQuarters` 는 처음부터 1-indexed 였음, 시트만
  어긋나 있던 상태)

**트레이드오프**:
- 초기 `_isJoined` 상태가 로컬 false — 다른 디바이스에서 이미 참가했거나
  앱 재실행 직후엔 실 상태가 반영 안 됨. 서버 participations 에서 derive
  하는 것은 다음 스프린트 과제
- Long-press 는 발견성 낮음 — 필요 시 툴팁/온보딩 힌트 별도 고려
